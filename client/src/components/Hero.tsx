...
const Hero: React.FC = () => {
  ...
  return (
    <section id="home" className="relative w-full h-[60vh] md:h-[80vh] overflow-hidden" aria-roledescription="carousel" aria-label="Promotional content">
      <div className="w-full h-full">
        {slides.map((slide, index) => (
          ...
          >
            ...
            <img src={slide.image} alt="" className="md:w-full h-full object-cover" />
      ...
      <button 
        onClick={prevSlide} 
        className="absolute top-1/2 left-4 md:left-8 transform -translate-y-1/2 z-30 bg-white/80 hover:bg-white rounded-full p-2 md:p-3 transition-colors text-gray-800"
      ...
      <button 
        onClick={nextSlide} 
        className="absolute top-1/2 right-4 md:right-8 transform -translate-y-1/2 z-30 bg-white/80 hover:bg-white rounded-full p-2 md:p-3 transition-colors text-gray-800"
      ...
      {/* Pagination Dots */}
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-30 flex space-x-3" role="group" aria-label="Slide indicator">
        {slides.map((_, index) => (
          <button
            key={index}
